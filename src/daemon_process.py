...
def github_job(subscription_manager, github_client, report_generator, notifier, days):
    ...
    """
    config = Config()
    report_type = config.report_types[0]
    ...
    for repo in subscriptions:
        ...
        report, report_file_path = report_generator.generate_daily_report(markdown_file_path,report_type)
        notifier.notify(repo, report,report_type)  # 发送通知
...
def hackernews_job(hackernews_client, report_generator, notifier):
    ...
    """
    config = Config()
    report_type = config.report_types[1]
    LOG.info("[开始执行Hacker News定时任务]")
    nmarkdown_file_path = hackernews_client.fetch_top_stories()  # 抓取最新的 Hacker News 头条新闻
    report, report_file_path = report_generator.generate_daily_report(nmarkdown_file_path,report_type)  # 生成 Hacker News 报告
    notifier.notify("Hacker News", report, report_type)  # 发送 Hacker News 报告通知
...
def main():
    ...
    hackernews_client = HackerNewsClient(config)
