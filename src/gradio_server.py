...
def export_progress_by_date_range(repo, days, send_email=False):
    try:
        ...
        email_status = "未发送"
        if send_email:
            notifier.notify(subject=repo, report=report, report_type="GitHub")  # 发送报告邮件
            email_status = "发送成功"
...
def generate_hackernews_report(send_email=False):
    try:
        ...
        news_content = hackernews_client.fetch_top_stories()  # 获取 Hacker News 头条新闻
        report, report_file_path = report_generator.generate_hackernews_report(news_content)  # 生成 Hacker News 报告
        LOG.info(f"Hacker News 报告生成成功: {report_file_path}")

        email_status = "未发送"
        if send_email:
            notifier.notify(subject="Hacker News", report=report, report_type="Hacker News")  # 发送报告邮件
            email_status = "发送成功"
